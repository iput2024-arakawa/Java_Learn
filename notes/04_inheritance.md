...
## 5. 【発展】`super(...)`が最初でなければならない理由と、その例外

### なぜ最初でなければならないのか

Javaのオブジェクトは「親クラスの部分が完成してから、子クラスの部分に取りかかる」
という順番で組み立てられます。もし親の部分が未完成のうちに、子クラスのコンストラクタが
親由来のフィールドやメソッドに触ってしまうと、初期化されていない値を読んでしまう
危険があります。そのためJavaは「親由来のもの（フィールド・メソッド）には、親の
コンストラクタ（`super(...)`）が終わるまで一切触らせない」というルールを、
コンパイル時にチェックしています。

```java
class Child extends Parent {
    Child(int base, int extra) {
        System.out.println(getBase()); // 親のメソッド呼び出し
        super(base);
        this.extra = extra;
    }
}
```
```
エラー: getBase()への参照は、明示的なコンストラクタの呼出しの後にのみ表示されます
```

### 例外（JDK 25/26以降の新機能：Flexible Constructor Bodies）

比較的新しいJavaバージョンでは、「親には一切触れない範囲」に限って
`super(...)`より前に文を書けるようになりました。

```java
public Bird(String name, int age, boolean canFly) {
    this.canFly = canFly;   // 自分自身（Bird）が宣言したフィールドへの代入はOK
    super(name, age);
}
```

許されるのは、**自分自身が宣言したフィールドへの代入**や**引数だけを使った計算**のみで、
親由来のフィールド・メソッドへのアクセスは今まで通り禁止です。多くの教材や過去の
Javaバージョンではまだ使えない書き方なので、初学者のうちは
**「`super(...)`は必ずコンストラクタの最初の1文」というスタイルに統一しておく**のが安全です。
